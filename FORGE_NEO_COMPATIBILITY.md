...
* **Event placeholders use typed input components.** Modern Gradio preprocesses
  every event input through its component data model. Task IDs now use a hidden
  `Textbox`, and the tab index uses a hidden `Number`; the removed hidden `Label`
